...
function Update:Window(text,logo,keybind)
    ...
    BtnStroke.Color = Color3.fromRGB(5, 254, 5)
...
Page.Image = ""
    ...
    function uitab:AddTab(text,img)
        ...
        NotificationHold.BackgroundColor3 = Color3.new(5, 254, 5)
        ...
        NotificationTitle.TextColor3 = Color3.fromRGB(5, 254, 5)
    ...
    function main:AddButton(text, callback)
        ...
        Button.BackgroundColor3 = Color3.fromRGB(5, 254, 5)
    ...
    function main:AddToggle(TogInfo ,default, callback)
        ...
        UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        UIStroke.Color = Color3.fromRGB(5, 254, 5)
    ...
    function main:AddDropdown(text,option,callback)
        ...
        DropScroll.BackgroundColor3 = Color3.fromRGB(5, 254, 5)
        ...
        DropButton.BackgroundColor3 = Color3.fromRGB(5, 254, 5)
    ...
    function main:AddSlider(text, min, max, start, callback)
        ...
        SliderFrame.BackgroundColor3 = Color3.fromRGB(5, 254, 5)
        ...
        SliderCount.BackgroundColor3 = Color3.fromRGB(5, 254, 5)
        ...
        SliderStroke.Color = Color3.fromRGB(5, 254, 5)
    ...
    function main:AddTextbox(text,disappear,callback)
        ...
        Textbox.BackgroundColor3 = Color3.new(5, 254, 5)
    ...
    function main:AddTextbox(text,disappear,callback)
        ...
        TextboxLabel.Name = "TextboxLabel"
        TextboxLabel.Parent = Textbox
        TextboxLabel.BackgroundColor3 = Color3.fromRGB(5, 254, 5)
        TextboxLabel.BackgroundTransparency = 1.000
        TextboxLabel.Position = UDim2.new(0, 15, 0, 0)
        TextboxLabel.Text = text
        TextboxLabel.Size = UDim2.new(0, 145, 0, 31)
        ...
        RealTextbox.Font = Enum.Font.GothamSemibold
        RealTextbox.Text = ""
        RealTextbox.TextColor3 = Color3.fromRGB(5, 254, 5)
        RealTextbox.TextSize = 11.000
        RealTextbox.TextTransparency = 0
    ...
    function main:AddSeperator(text)
        ...
        Sep1.BackgroundColor3 = Color3.new(5, 254, 5)
        ...
        Sep3.BackgroundColor3 = Color3.new(5, 254, 5)
    ...
    function main:AddLine()
        ...
        Line.Name = "Line"
        Line.Parent = Linee
        Line.BackgroundColor3 = Color3.new(5, 254, 5)
        Line.BorderSizePixel = 0
			...
			if v.Name == "Flower2" or v.Name == "Flower1" then
				if FlowerESP then 
					if not v:FindFirstChild('NameEsp'..Number) then
						...
						name.BackgroundTransparency = 1
						name.TextStrokeTransparency = 0.5
						name.TextColor3 = Color3.fromRGB(5, 254, 5)
...
-- 初始化核心模块变量（默认赋值为空函数，避免后续调用报错）
local STOP = { play = function() end }  -- 兜底空函数
local STOPRL = {
    wrapAttackAnimationAsync = function(...) end,
    getBladeHits = function(...) return nil end
}

-- 尝试加载 PlayerScripts 中的 Particle 模块（失败则使用兜底函数）
local PlayerScripts = Client:FindFirstChild("PlayerScripts")
if PlayerScripts then
    local CombatFramework = PlayerScripts:FindFirstChild("CombatFramework")
    if CombatFramework then
        local ParticleModule = CombatFramework:FindFirstChild("Particle")
        if ParticleModule then
            local success, result = pcall(require, ParticleModule)
            if success then STOP = result end
        end
    end
end

-- 尝试加载 ReplicatedStorage 中的 RigLib 模块（失败则使用兜底函数）
local RSCombatFramework = ReplicatedStorage:FindFirstChild("CombatFramework")
if RSCombatFramework then
    local RigLibModule = RSCombatFramework:FindFirstChild("RigLib")
    if RigLibModule then
        local success, result = pcall(require, RigLibModule)
        if success then STOPRL = result end
    end
end

-- 核心动画修改逻辑（即使模块加载失败，也不会报错）
spawn(function()
    while task.wait() do
        pcall(function()
            -- 保存原始函数（避免重复覆盖）
...
-- 可选：添加运行提示，确认脚本已执行
warn("核心战斗逻辑脚本已加载完成")
if STOP.play == nil or type(STOP.play) ~= "function" then
    warn("提示：Particle模块加载失败，但脚本仍会尝试运行核心逻辑")
end
if STOPRL.wrapAttackAnimationAsync == nil or type(STOPRL.wrapAttackAnimationAsync) ~= "function" then
    warn("提示：RigLib模块加载失败，但脚本仍会尝试运行核心逻辑")
end----------
---------------------------------------------
---
---
...
while wait(.1) do
    if _G.FastAttack then
       ...
       Main:AddToggle("Đặt điểm hồi sinh",true,function(value)
    _G.Set = value
end)
spawn(function()
   while wait() do
...
Main:AddToggle("Tấn công nhanh",true,function(value)
   ...
   Main:AddToggle("Đánh quái ở gần ",_G.AutoFarmNearest,function(value)
...
Main:AddToggle("Cày rương | An toàn ",false,function(value)
 AutoFarmChest = value
...
Main:AddToggle("Cày rương | Rủi ro bị kick",false,function(vu)
	_G.ChestBypass = vu
end)
...
if _G.ChestBypass then
    ...
    Main:AddButton("Di chuyển nhanh đến đảo bánh",function()
        if BypassTP then
    ...
    Main:AddToggle("Mở cổng",true,function(value)
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner",value)
    end)
    
    Main:AddToggle("Cày tư lệnh bột V2", _G.Autodoughking,function(value)
...
Main:AddToggle("Cày boss",_G.AutoFarmBoss,function(value)
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
    ...
    Main:AddToggle("Nhận nhiệm vụ (thử nghiệm)",false,function(value)
...
spawn(function()
    while task.wait(0.1) do  -- 替换wait为task.wait，更稳定
        if _G.AUTOHAKI then 
            -- 增加Character和HasBuso的安全检查，避免空值报错
            local LocalPlayer = game.Players.LocalPlayer
            if LocalPlayer and LocalPlayer.Character then
                if not LocalPlayer.Character:FindFirstChild("HasBuso") then
                    local args = { [1] = "Buso" }
                    -- 安全调用Remote函数，增加pcall防护
                    pcall(function()
...
-- 2. 超快速攻击开关
Main:AddToggle("Tấn công nhanh (siêu)", false, function(value)
    _G.HyperSonic = value
end)      

-- 3. 核心修复：安全加载模块，避免CombatFramework不存在时报错
...
-- 安全加载CombatFramework（核心报错修复点）
local LocalPlayer = game.Players.LocalPlayer
if LocalPlayer then
    local PlayerScripts = LocalPlayer:FindFirstChild("PlayerScripts")
    if PlayerScripts then
        local CombatFrameworkModule = PlayerScripts:FindFirstChild("CombatFramework")
        if CombatFrameworkModule then
            pcall(function()
                CombatFrameworkR = require(CombatFrameworkModule)
                -- 安全获取upvalue，避免索引越界
                local upvalues = debug.getupvalues(CombatFrameworkR)
                if #upvalues >= 2 then
                    y = upvalues[2]
                end
            end)
...
-- 4. 攻击速度修改逻辑（仅在y存在时执行）
spawn(function()
    if game:GetService("RunService"):FindFirstChild("RenderStepped") then
        game:GetService("RunService").RenderStepped:Connect(function()
            if (_G.FastAttack or _G.HyperSonic) and y then
                if typeof(y) == "table" then
                    pcall(function()
                        if CameraShaker then
                            CameraShaker:Stop()
                        end
                        -- 增加activeController的存在性检查
        ...
        if _G.FastAttack == true or _G.HyperSonic == true then
            ...
            if LocalPlayer and LocalPlayer.Character then
                ...
                pcall(function()
                    ...
                    if LocalPlayer.Character:FindFirstChild("Busy") then
    ...
    while task.wait(0.1) do
        if _G.HyperSonic then
            pcall(function()
                repeat 
                    task.wait(0.1)
                    -- 检查AttackHit函数是否存在，避免调用不存在的函数报错
                    if typeof(AttackHit) == "function" then
                        AttackHit()
                    end
...
Main:AddToggle("Xoá thông báo lên cấp",false,function(Remo)
_G.RemoveHit = Remo
end)
...
spawn(function()
    ...
    Main:AddToggle("Xoá hành động tấn công",false,function(value)
    getgenv().A = require(game:GetService("ReplicatedStorage").CombatFramework.RigLib).wrapAttackAnimationAsync 
getgenv().B = require(game.Players.LocalPlayer.PlayerScripts.CombatFramework.Particle).play
spawn(function()
    ...
    spawn(function()
      ...
      if _G.WhiteScreen then
        ...
        Main:AddToggle("Màn hình treo",_G.WhiteScreen,function(value)
...
elseif _G.WhiteScreen == false then
    ...
    PosX = 1
    Main:AddSlider("Ngang",0,50,1,function(value)
    PosX = value
    end)

    PosY = 30
    Main:AddSlider("Trên",0,50,45,function(value)
    PosY = value
    end)

    PosZ = 10
    Main:AddSlider("Ngang",0,50,40,function(value)
    PosZ = value
    end)
    ...
    Main:AddSlider("Máu Quái",1,100,35,function(value)
...
do
    ...
    if World1 then
        M:AddToggle("Qua Sea 2",_G.AutoSecondSea,function(value)
    ...
    if World2 then
        ...
        M:AddToggle("Qua Sea 3",_G.AutoThirdSea,function(value)
...
M:AddToggle("Bay đến đảo kì bí",_G.AutoMysticIsland,function(value)
        ...
        spawn(function()
            ...
            M:AddButton("Bay đến bán trái cây (xịn)", function()
                TweenNpc()
                end)
    ...
    M:AddToggle("Treo đảo kì bí (thử nghiệm)",_G.dao,function(state)
    if state then
        _G.dao = true
    else
        _G.dao = false
...
M:AddToggle("Bay đến bánh răng",_G.TweenMGear,function(value)
_G.TweenMGear = value
...
spawn(function()
    ...
    M:AddToggle("Lấy Superhuman",_G.AutoSuperhuman,function(value)
    ...
    M:AddToggle("Lấy GodHuman",_G.Auto_God_Human,function(value)
...
M:AddToggle("Cày Radioactive Material",false,function(value)
Radioactive = value
...
M:AddToggle("Cày Vampire Fang",false,function(value)
BjirVampire = value
...
spawn(function()
    ...
    M:AddToggle("Cày Mystic Droplet",false,function(value)
_G.Makori_gay = value
...
spawn(function()
    ...
    M:AddToggle("Cày Magma Ore",_G.Makori_gay,function(value)
_G.Umm = value
StopTween(_G.Umm)
end)
...
spawn(function()
    ...
    M:AddToggle("Cày Angel Wings",_G.Makori_gay,function(value)
_G.Auto_Wing = value
...
spawn(function()
    ...
    M:AddToggle("Cày Leather",_G.Makori_gay,function(value)
_G.Leather = value
...
spawn(function()
    ...
    M:AddToggle("Cày Scrap Metal",false,function(value)
Scrap = value
StopTween(Scrap)
end)
...
spawn(function()
    ...
    M:AddToggle("Cày bột ca cao",false,function(value)
Cocoafarm = value
StopTween(Cocoafarm)
end)
...
spawn(function()
    ...
    M:AddToggle("Cày Gunpowder",false,function(value)
Gunpowder = value
StopTween(Gunpowder)
end)
...
spawn(function()
    ...
    M:AddToggle("Cày Fish Tail Sea 3",false,function(value)
Fish = value
StopTween(Fish)
end)
...
spawn(function()
    ...
    M:AddToggle("Cày Mini Tusk",false,function(value)
MiniHee = value
StopTween(MiniHee)
end)
...
M:AddToggle("Cày cấp trái ác quỷ",_G.AutoFarmFruitMastery,function(value)
...
spawn(function()
    ...
    M:AddToggle("Cày cấp súng",_G.AutoFarmGunMastery,function(value)
    ...
    M:AddToggle("Lấy Buddy Sword",_G.AutoBudySword,function(value)
        _G.AutoBudySword = value
        StopTween(_G.AutoBudySword)
    end)
    
    M:AddToggle("Lấy Buddy Sword Hop",_G.AutoBudySwordHop,function(value)
    ...
    M:AddToggle("Cày kiếm Law", _G.AutoOderSword,function(value)
    ...
    task.spawn(function()
            while task.wait() do
                pcall(function()
                    if game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=137174788979709149431" then
                        FM:Set("ðŸŒ‘: Full Moon 100%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=137174788979709149052" then
                        FM:Set("ðŸŒ’: Full Moon 75%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=137174788979709143733" then
                        FM:Set("ðŸŒ“: Full Moon 50%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=137174788979709150401" then
                        FM:Set("ðŸŒ”: Full Moon 25%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=137174788979709149680" then
...
M:AddLabel("Không hoạt động trơn tru")
...
M:AddToggle("Lấy song kiếm nguyền rủa",TestCDK,function(value)
TestCDK = value
...
M:AddToggle("Lấy Tushita", _G.Autotushita,function(value)
    ...
    M:AddToggle("Cày hải tặc", _G.AutoRaidPirate,function(value)
...
spawn(function()
    ...
    M:AddToggle("Cày boss bí ẩn",_G.AutoElitehunter,function(value)
    ...
    M:AddToggle("Lấy Hallow Scythe",_G.AutoFarmBossHallow,function(value)
    ...
    M:AddToggle("Đánh boss Rip_Indra",_G.AutoDarkDagger,function(value)
...
M:AddToggle("Triệu hồi Rip_Indra (cần chén thánh)",false,function()
AutoHaki()
wait(0.5) 
local args = {
[1] = "activateColor",
...
HyperCahaya(CFrame.new(-5419.98095703125, 1093.3375244140625, -2666.730712890625))
	...
	M:AddToggle("Lấy Dragon Trident", _G.Auto_Dragon_Trident,function(value)
	...
	M:AddToggle("Lấy Waden Sword", _G.Autowaden,function(value)
         _G.Autowaden = value
    ...
    M:AddToggle("Đánh râu trắng", _G.Autodoughking,function(value)
         _G.Autogay = value
    ...
    M:AddToggle("Lấy Pole v1", _G.Autodoughking,function(value)
         _G.Autopole = value
    ...
    M:AddToggle("Lấy Swan Glasses",_G.AutoFarmSwanGlasses,function(value)
    ...
    M:AddToggle("Lấy Sharks saw", _G.Autodoughking,function(value)
         _G.Autosaw = value
        StopTween( _G.Autopole)
    end)
    ...
    M:AddToggle("Trao đổi xương",_G.Auto_Random_Bone,function(value)
 ...
 spawn(function()
    ...
    M:AddToggle("Lấy Rainbow Haki",_G.Auto_Rainbow_Haki,function(value)
    ...
    M:AddToggle("Tự động Citizen Quest",_G.AutoObservationHakiV2,function(x)
    ...
    M:AddToggle("Tự động Quest Evo Race (V2)",_G.Auto_EvoRace,function(value)
...
Ss:AddToggle("Tự động nâng",_G.Auto_Stats_Kaitun,function(value)
...
Ss:AddToggle("Máu",false,function(value)
defense = value
end)
...
Ss:AddToggle("Trái",false,function(value)
demonfruit = value
end)

PointStats = 1
Ss:AddSlider("Điểm số",1,100,PointStats,nil,function(value)
PointStats = value
end)
...
K:AddToggle("Khởi đầu kitsune",false,function(vu)
    _G.SummonKitsune = vu
...
K:AddToggle("Đến đảo kitsune",false,function(vu)
    _G.Kitsune = vu
...
K:AddToggle("Thu thập hồn lửa",false,function(vu)
    _G.AzureKitsune = vu
...
K:AddToggle("Trao đổi hồn lửa",_G.TradeItem,function(state)
...
K:AddToggle('Đánh Sea Beast (thử nghiệm)', false, function(value)
...
K:AddToggle('Đánh PirateBrigade', _G.bjirPirateBrigade, function(value)
...
K:AddToggle('Đánh GhostShip', _G.bjirFishBoat, function(value)
...
K:AddToggle("Đánh Terrorshark",_G.Makori_gay,function(value)
...
K:AddToggle("Đánh Shark",_G.Makori_gay,function(value)
_G.bjirShark = value
...
K:AddToggle("Đánh Piranha",_G.Makori_gay,function(value)
...
K:AddToggle("Tự động ra biển 6",_G.AutoRemover,function(state)
if state then
    _G.AutoRemover = true
else
...
RaceV4:AddButton("Đến npc bán bánh răng (Must Be in Temple Of Time!)",function()
  HyperCahaya(CFrame.new(28981.552734375, 14888.4267578125, -120.245849609375))
end)
   
   RaceV4:AddButton("Bẻ khoá cần gạt.", function()
...
game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer(unpack(args))
  ...
  RaceV4:AddButton("Đến cửa",function()
    Game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(28286.35546875, 14895.3017578125, 102.62469482421875)
    wait(.1)
    Game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(28286.35546875, 14895.3017578125, 102.62469482421875)
    wait(.1)
  ...
  RaceV4:AddToggle("Hoàn thành ải",false,function(v)
  _G.AutoQuestRace = v
...
RaceV4:AddToggle("Tiêu diệt người chơi!",false,function(value)
...
spawn(function()
  ...
  RaceV4:AddButton("Mua bánh răng",function(t)
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer('UpgradeRace','Buy')
    ...
    P:AddToggle("Giết người chơi",false,function(value)
...
spawn(function()
    ...
    R:AddToggle("Nâng V2",_G.Auto_Awakener,function(value)
    ...
    R:AddToggle("Giết quái",_G.concubu ,function(vu)
    _G.concubu  = vu
    end)
...
spawn(function()
    ...
    R:AddToggle("Vào tập kích",_G.Auto_StartRaid,function(value)
    ...
    T:AddButton("Đến sea 1",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelMain")
    end)
    
    T:AddButton("Đến sea 2",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
    end)
    
    T:AddButton("Đến sea 3",function()
    ...
    T:AddToggle("Di chuyển",_G.TeleportNPC,function(value)
    ...
    T:AddButton("Di chuyển nhanh",function()
...
S:AddDropdown("Chọn kĩ năng",AbiList,function(vu)
    _G.Abilities = vu
end)
...
S:AddButton("Mua kĩ năng",function()
...
S:AddDropdown("Chọn Melee",FightingStyle,function(vu)
    _G.BuyMelee = vu
end)
...
S:AddButton("Mua cận chiến",function()
...
S:AddDropdown("Chọn kiếm",SwordStyle,function(vu)
    _G.SwordStyle = vu
end)

S:AddButton("Mua kiếm",function()
    ...
    S:AddButton("Mua Slingshot [ $5,000 Beli ]",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem","Slingshot")
    end)
    
    S:AddButton("Mua Musket [ $8,000 Beli ]",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem","Musket")
    end)
    
    S:AddButton("Mua Flintlock [ $10,500 Beli ]",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem","Flintlock")
    end)
    
    S:AddButton("Mua Refined Slingshot [ $30,000 Beli ]",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem","Refined Flintlock")
    end)
    
    S:AddButton("Mua Refined Flintlock [ $65,000 Beli ]",function()
		local args = {
			[1] = "BuyItem",
			[2] = "Refined Flintlock"
		}
    ...
    S:AddButton("Mua Cannon [ $100,000 Beli ]",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem","Cannon")
    end)
    
    S:AddButton("Mua Kabucha [ 1,500 Fragments]",function()
...
S:AddButton("Hoàn trả chỉ số (Use 2.5K Fragments)", function()
...
S:AddButton("Đổi tộc (Use 3K Fragments)", function()
	...
	S:AddButton("Mua Black Cape [ $50,000 Beli ]",function()
		local args = {
			[1] = "BuyItem",
			[2] = "Black Cape"
		}
	...
	S:AddButton("Mua Swordsman Hat [ 150k Beli ]",function()
		local args = {
			[1] = "BuyItem",
			[2] = "Swordsman Hat"
		}
	...
	S:AddButton("Mua Tomoe Ring [ $500k Beli ]",function()
		local args = {
			[1] = "BuyItem",
			[2] = "Tomoe Ring"
		}
...
D:AddSeperator("Sniper")
    ...
    _G.SelectFruit = ""
    D:AddDropdown("Chọn trái",Table_DevilFruitSniper,function(value)
        _G.SelectFruit = value
    end)
    
    D:AddToggle("Mua trái ác quỷ",_G.AutoBuyFruitSniper,function(value)
        _G.AutoBuyFruitSniper = value
    end)
    
        D:AddButton("Kiểm tra trái ác quỷ bán",function()
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
closeButton.MouseButton1Click:Connect(closeGUI)
    ...
    D:AddButton("Trao đổi trái",function()
...
spawn(function()
    ...
    D:AddToggle("Nhặt trái ác quỷ (dễ kick) ",_G.Grabfruit,function(value)
    _G.Grabfruit = value
    end)
    spawn(function()
			while wait(.1) do
    ...
    D:AddToggle("Bay đến trái ác quỷ (nên dùng)",_G.Tweenfruit,function(value)
    _G.Tweenfruit = value
    ...
    Misc:AddButton("Vào lại máy chủ của bạn",function()
        game:GetService("TeleportService"):Teleport(game.PlaceId, game:GetService("Players").LocalPlayer)
    end)
    
    Misc:AddButton("Vào máy chủ khác",function()
        Hop()
    end)
    
    Misc:AddButton("Vào máy chủ ít người",function()
    ...
    Misc:AddButton("Sao chép ID máy chủ của bạn",function()
	setclipboard(tostring(game.JobId))
	end)
	
    Misc:AddTextbox("Nhập ID máy chủ", true, function(value)
	_G.Job = value
	end)
    ...
    Misc:AddButton("Mở cửa hàng trái ác quỷ",function()
...
Misc:AddButton("Mở bảng màu haki",function()
...
Misc:AddSeperator("Teams")
    ...
    Misc:AddButton("Vào hải quân",function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("SetTeam","Marines") 
    end)
    
    Misc:AddSeperator("Highlight")

    Misc:AddToggle("Tối giản",false,function(value)
        if value == true then
...
Misc:AddToggle("Đồ hoạ",false,function(v)
	...
	Misc:AddToggle("Kiếm Yoru 7 màu",false,function(value)
		RainbowYoru = value
	end)
...
Misc:AddToggle("Tránh bị văng", true, function()
...
repeat wait() until game:IsLoaded() 
	...
	Misc:AddToggle("Vào lại máy chủ khi bị văng",true,function(value)
		_G.AutoRejoin = value
	end)
    ...
    Misc:AddButton("Mở Các cổng dịch chuyển",function()
...
Misc:AddButton("Hiện vật phẩm", function(value)
    ...
    Misc:AddButton("Nhập hết code X2",function()
    ...
    Misc:AddButton("Giảm Lag (nhẹ)",function()
        pcall(function()
    ...
    Misc:AddToggle("Ẩn Sát thương",function()
		for i,v in pairs(game.Workspace:GetDescendants()) do
    ...
    Misc:AddButton("Giảm Lag", function()
...
Misc:AddSeperator("ESP Menu")
    ...
    Misc:AddToggle("ESP Trái ác quỷ",false,function(a)
        DevilFruitESP = a
...
Misc:AddToggle("Esp Quái", false, function(nec)
    ...
    Misc:AddToggle("Lướt vô hạn",false,function(value)
        nododgecool = value
        NoDodgeCool()
    end)
    
    Misc:AddToggle("Bật tộc V3",_G.AutoAgility,function(value)
    ...
    Misc:AddToggle("Vô hạn Soru",getgenv().InfSoru,function(value)
